...
func _show_category_display_label(ui_inventory_category : Node = null) -> void:
	...
	var label_half_width = _label_category_name.get_rect().size.x / 4 + 5
...
func _setup_context_menu() -> void:
	...
	var x = middle_category.get_global_position().x + (middle_category.get_size().x / 4) - (_context_menu.get_size().x / 10 - 130)	
